...
const contestsListEl = $('#contests-list');
...
async function loadContests() {
  if (!hasStoreSlug()) return;
  try {
    const data = await fetchContests(getSlug());
...
function renderJobOpenings() {
  ...
  if (!jobOpenings.length) {
    jobOpeningsListEl.innerHTML = emptyStateHtml({
      title: 'Nuk ka konkurse pune aktive',
      text: 'Kontrolloni më vonë për pozicione të reja.',
    });
    jobOpeningsEmptyEl?.classList.add('hidden');
    bindEmptyStateActions(jobOpeningsListEl);
    return;
  }
  jobOpeningsEmptyEl?.classList.add('hidden');
...
async function loadJobOpenings() {
  if (!hasStoreSlug()) return;
  try {
    const data = await fetchJobOpenings(getSlug());
